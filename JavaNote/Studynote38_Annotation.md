...
class Subclass extends Superclass{
    //注解
    //1. @Override 注解在fly方法上，表示子类重写。如果不写这个注解，依然构成重写
...
2. 如果不写 @Override 注解，而父类仍有该方法，仍然构成重写，只是不再编译检查
3. 查看@Override注解源码为 @Target(ElementType.METHOD) 
   * 说明：只能修饰方法，不能修饰其它类，包，属性等等
...
3. @Target(value={CONSTRUCTOR, FIELD, LOCAL_VARIABLE, METHOD, PACKAGE, PARAMETER, TYPE})
      ...
      * 源代码说明可修饰：构造器   字段(属性)    局部变量           方法      包       参数     类型

4. @Deprecated可以做到新旧版本的兼容和过渡。表示不再推荐使用，但仍然可以使用



### @SuppressWarnings

1. 当我们不希望看到警告的时候，可以使用 @Suppresswarnings 注解来抑制警告信息
...
4. 关于 SuppressWarnings 作用范围，和放置的位置相关

   * 比如 @SuppressWarnings放置在 main 方法，那么抑制警告的范围就是 main
   * 通常我们可以放置具体的语句, 方法, 类.
   
5.  @SuppressWarnings 源码解读
...
用于指定被修饰的Annotation能用于修饰哪些程序元素，也包含一个名为value的成员变量。
...
@Retention 中可使用的值定义在 RetentionPolicy 枚举类中，指定其保留时间。常用值如下

- RetentionPolicy.SOURCE：注解只作用在源码阶段，编译器生成的字节码文件中不存在，直接丢弃。
- RetentionPolicy.CLASS：注解作用在源码阶段和字节码文件阶段，运行阶段不存在，不会被加载到 JVM 中，默认值
- RetentionPolicy.RUNTIME：注解作用在源码阶段，字节码文件阶段，运行阶段（开发常用）。程序可以通过反射获取该注释
...
被它修饰的 Annotation 具有继承性

如果一个超类被 @Inherited 修饰，如果子类没有被任何注解应用时，该子类就继承超类的注解
...
```java
// 格式模板
public @interface 注解名称 { //@interface不是interface，是在jdk5.0之后加入的注解类
...
@MyAnnotation("张三") //只有一个 value 属性，或其他属性有默认值，则可以省略属性名称
```
